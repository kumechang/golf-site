...
2. **画像素材の追加**: `src/assets/photos/` にトップページのヒーロー・「大会・コミュニティ情報」「上達・ルール」カテゴリページ用の写真を3枚配置済み（Unsplash/Pexels/Pixabayの無料枠から、実際の写真を目視確認のうえ厳選）。収集した素材の大半は「グラウンドゴルフ」で検索しても実際は別の競技（本物のゴルフ・クリケット・サッカー等）や無関係の写真だったため、安全に使えるものはこの3枚にとどまっています。記事本文用の画像や他カテゴリの画像は引き続き未挿入です。追加する場合は `src/assets/photos/` に配置し、`src/data/category-images.ts` 等で差し込んでください（画像は必ず目視確認のうえ、Unsplash+等の有料素材でないことをmanifest.jsonの`imageUrl`で確認してから使用すること）
